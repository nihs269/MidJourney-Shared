import discord
import Globals
from Salai import pass_prompt_to_self_bot, upscale, max_upscale, variation
...
async def hello(ctx, sentence: discord.Option(str)):
    await ctx.respond("I'm ready to draw something!!")
...
async def mj_imagine(ctx, prompt: discord.Option(str)):
    ...
    response = pass_prompt_to_self_bot(prompt)
...
async def mj_upscale_to_max(ctx):
    ...
    response = max_upscale(Globals.targetID, Globals.targetHash)
    Globals.targetID = ""
    if response.status_code >= 400:
        await ctx.respond("Request has failed; please try later")
        return
